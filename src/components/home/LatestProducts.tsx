...
import { IErrorResponse, TProductResponse } from "types/types";
...
const LatestProducts = () => {
  const { data, isError, isLoading, error } = useGetLatestSportsQuery(undefined, {pollingInterval:30000, skip:false});
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 md:my-6">
          {data?.data.map((product, index) => {
            return (
              <motion.div animate={{rotate:[0, 90, 90, 0], scale:[1, 0.50, 0.75 , 1]}} key={index}>
