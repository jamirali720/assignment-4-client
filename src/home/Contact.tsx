...
const Contact = () => {
  ...
  const [contactEmailSend, { isSuccess, isLoading }] = useContactEmailSendMutation();
   ...
   const onSubmit: SubmitHandler<TContact> = async (data) => {    
      try {
       await contactEmailSend(data);
        ...
        console.error(error)
        toast.error('Failed to send email. Please try again!', {position: "top-center", id:2, duration: 2000});
      }
            ...
            <div className="my-5 md:border-l">
              <form className="md:px-10" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className=" w-11/12 my-5 text-end">
                  <button
                    type="submit"
                    className="bg-slate-100 hover:bg-slate-200 w-3/12 px-5 py-2 font-semibold rounded-full text-[#F45634] "
                  >
