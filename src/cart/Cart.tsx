...
import { cartItems,setTotalAmount, decrementItem, incrementItem, removeItem, clearCartItems } from "../redux/features/cartSlice";
import { useDispatch } from "react-redux";
import { Card } from "antd";

const Cart = () => {
  const cartItemsProducts = useAppSelector(cartItems);
  const subtotal = cartItemsProducts.reduce((sum, item) => sum + item.price * item.quantity, 0)
  // const totalAmount = useAppSelector(state => state.cart.totalAmount);
  const shippingCharge =   subtotal === 0  ?  0 : subtotal >= 5000 ? 100 : 200; 
  const texCharge = subtotal * 0.10;
          ...
          <div className="w-screen md:w-full max-w-full my-4 overflow-auto">
                      ...
                      >
                        ...
                        <td className="border border-slate-200">
                          <button
                            ...
                            onClick={() => handleRemoveCartItems(product._id!)}
                          >
                            Remove
                          </button>
                        </td>
            ...
            <div className="my-3 flex justify-between">
              ...
              <button
                ...
                className="bg-slate-100 hover:bg-slate-200 px-5 py-2 font-semibold rounded-full text-red-500"
              >
                Clear Cart
              </button>
            </div>
        ...
        >
          ...
          <div className="bg-red-200">
            <button
              ...
              className="bg-slate-100 float-end  hover:bg-slate-200  ease-in-out duration-300 px-5 w-full py-2 rounded-full "
            >
              <Link
                to="/shipment"
                className="block text-[#F45634] font-semibold text-1xl "
