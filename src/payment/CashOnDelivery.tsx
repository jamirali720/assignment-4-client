...
const CashOnDelivery = () => { 
  ...
  const dispatch = useAppDispatch();
    const cartOrders = useAppSelector(cartItems)
const navigate = useNavigate();
    const orders = cartOrders?.map((item: Inputs) => {
    ...
    useEffect(() => {
      if(isSuccess) {        
        navigate("/")
        toast.success("Success! You have successfully Updated stock with cash on delivery", {position: "top-center", duration: 5000})
        ...
        <div className="w-screen h-full flex justify-center justify-items-center">
          <div className="flex items-center">
            <div className="border p-6 ">
              <AiFillCheckCircle className="my-5  mx-auto" color="green" size={70} />
              <h3 className="my-3 text-3xl font-semibold text-center">Thank you for your order!</h3>
              ...
              <div className="text-center">
                <button className="w-64 h-10 rounded-sm bg-red-400 hover:bg-red-500 transition-all duration-300 ease-linear text-white font-semibold" onClick={handleBackToHome}>Back To Home</button>
              </div>
            </div>
          </div>
        </div>
